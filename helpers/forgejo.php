...
$fjtoken  = $config['forgejoToken'] ?: null;
$fjurl = $config['forgejoURL'];
...
if (!empty($config['allowedForgejoOwners']) && is_array($config['allowedForgejoOwners']) && !in_array($owner, $config['allowedForgejoOwners'], true)) {
...
function fj_cached_get(string $url, int $ttl, ?string $fjtoken = null, bool $withHeaders = false): ?array
{
    ...
    if ($fjtoken) $headers[] = 'Authorization: Bearer ' . $fjtoken;
...
function fj_repo_info(string $owner, string $repo, int $ttl, ?string $fjtoken): ?array
{
    global $fjurl;
    $url = "$fjurl/api/v1/repos/$owner/$repo";
    return fj_cached_get($url, $ttl, $fjtoken);
}
function fj_user_info(string $owner, int $ttl, ?string $fjtoken): ?array
{
    global $fjurl;
    $url = "$fjurl/api/v1/users/$owner";
    return fj_cached_get($url, $ttl, $fjtoken);
}
// ----------------- Release Infos --------------------
if (!function_exists('fj_releases_info')) {
    function fj_releases_info(string $owner, string $repo, int $ttl = 3600, ?string $fjtoken = null): ?array
    {
        global $fjurl;
        $url = "$fjurl/api/v1/repos/$owner/$repo/releases";
        return fj_cached_get($url, $ttl, $fjtoken);
    }
}

if (!function_exists('fj_latest_release')) {
    function fj_latest_release(string $owner, string $repo, int $ttl = 3600, ?string $fjtoken = null): ?array
    {
        $releases = fj_releases_info($owner, $repo, $ttl, $fjtoken);
...
// ----------------- Lizenz Infos --------------------
if (!function_exists('fj_licenses_info')) {
    function fj_licenses_info(int $ttl = 86400, ?string $fjtoken = null): ?array
    {
        global $fjurl;
        // List of all licenses from Codeberg / Forgejo
        $url = "$fjurl/api/v1/licenses";
        return fj_cached_get($url, $ttl, $fjtoken);
    }
}

if (!function_exists('fj_repo_license')) {
    function fj_repo_license(string $owner, string $repo, int $ttl = 3600, ?string $fjtoken = null): string
    {
        global $fjurl;
        // Get LICENSE file
        $url = "$fjurl/api/v1/repos/$owner/$repo/contents/LICENSE";
        $file = fj_cached_get($url, $ttl, $fjtoken);
        ...
        // Get license list
        $licenses = fj_licenses_info($ttl, $fjtoken);
...
// ----------------- Helpers --------------------
if (!function_exists('fj_is_issue')) {
    function fj_is_issue(array $item): bool
    {
...
if (!function_exists('fj_repo_issues_count')) {
    /**
     * @param string $owner
     * @param string $repo
     * @param string $state 'open'|'closed'|'all'
     * @param int $ttl Cache TTL
     * @param string|null $fjtoken
     * @return int
     */
    function fj_repo_issues_count(string $owner, string $repo, string $state = 'all', int $ttl = 300, ?string $fjtoken = null): int
    {
        global $fjurl;
        $url = "$fjurl/api/v1/repos/$owner/$repo/issues?state=" . rawurlencode($state) . "&limit=1";
        $resp = fj_cached_get($url, $ttl, $fjtoken, true);
        ...
        while (true) {
            $urlPage = "$fjurl/api/v1/repos/$owner/$repo/issues?state=" . rawurlencode($state)
                . "&limit=" . $perPage
                . "&page=" . $page;
            $pageData = fj_cached_get($urlPage, $ttl, $fjtoken);
            if (!$pageData || !is_array($pageData) || count($pageData) === 0) {
                break;
            }
            foreach ($pageData as $item) {
                if (is_array($item) && fj_is_issue($item)) {
                    $total++;
                }
            }
...
if (!function_exists('fj_user_public_repos')) {
    function fj_user_public_repos(string $owner, int $ttl = 300, ?string $fjtoken = null): array
    {
        global $fjurl;
        $url = "$fjurl/api/v1/users/$owner/repos?limit=100";
        $repos = fj_cached_get($url, $ttl, $fjtoken);
        return is_array($repos) ? $repos : [];
    }
}
if (!function_exists('fj_all_repos_issues_count')) {
    function fj_all_repos_issues_count(string $owner, string $state = 'all', int $ttl = 300, ?string $fjtoken = null): int
    {
        $repos = fj_user_public_repos($owner, $ttl, $fjtoken);
        $total = 0;
        foreach ($repos as $r) {
            if (!isset($r['name'])) continue;
            $total += fj_repo_issues_count($owner, $r['name'], $state, $ttl, $fjtoken);
        }
        return $total;
    }
}
...
// ----------------------------------------------------
if (!function_exists('fj_repo_prs_count')) {
    /**
     * Counts pull requests in a repo via the /pulls API.
     * Uses X-Total-Count header, fallback pagination.
     */
    function fj_repo_prs_count(string $owner, string $repo, string $state = 'all', int $ttl = 300, ?string $fjtoken = null): int
    {
        global $fjurl;
        $url = "$fjurl/api/v1/repos/$owner/$repo/pulls?state=" . rawurlencode($state) . "&limit=1";
        $resp = fj_cached_get($url, $ttl, $fjtoken, true);
        ...
        while (true) {
            $urlPage = "$fjurl/api/v1/repos/$owner/$repo/pulls?state=" . rawurlencode($state)
                . "&limit=" . $perPage
                . "&page=" . $page;
            $pageData = fj_cached_get($urlPage, $ttl, $fjtoken);
            if (!$pageData || !is_array($pageData) || count($pageData) === 0) {
...
// ----------------------------------------------------
if (!function_exists('fj_all_repos_prs_count')) {
    function fj_all_repos_prs_count(string $owner, string $state = 'all', int $ttl = 300, ?string $fjtoken = null): int
    {
        $repos = fj_user_public_repos($owner, $ttl, $fjtoken);
        $total = 0;
        foreach ($repos as $r) {
            if (!isset($r['name'])) continue;
            $total += fj_repo_prs_count($owner, $r['name'], $state, $ttl, $fjtoken);
        }
        return $total;
    }
}
...
// ----------------------------------------------------
function fj_get_all_repos(string $username, int $ttl, ?string $token = null): array
{
    global $fjurl;
    $page = 1;
    $perPage = 100;
    $all = [];
    do {
        $url = "$fjurl/api/v1/users/{$username}/repos?page={$page}&limit={$perPage}";
        $repos = fj_cached_get($url, $ttl, $token);
...
// ----------------------------------------------------
function fj_get_last_commit_info(string $owner, int $ttl, ?string $fjtoken = null): ?array
{
    global $fjurl;
    $repos = fj_get_all_repos($owner, $ttl, $fjtoken);
    $latestCommit = null;
    $latestTime = 0;
    ...
    foreach ($repos as $repo) {
        ...
        $branchesUrl = "$fjurl/api/v1/repos/{$owner}/{$repoName}/branches";
        $branches = fj_cached_get($branchesUrl, $ttl, $fjtoken);
        ...
        foreach ($branches as $branch) {
            ...
            $commitUrl = "$fjurl/api/v1/repos/{$owner}/{$repoName}/commits?sha=" . urlencode($branchName) . "&limit=1";
            $commits = fj_cached_get($commitUrl, $ttl, $fjtoken);
...
function fj_get_repo_milestones(string $owner, string $repo, int $ttl, ?string $fjtoken = null): ?array
{
    global $fjurl;
    $url = "$fjurl/api/v1/repos/{$owner}/{$repo}/milestones?state=all";
    return fj_cached_get($url, $ttl, $fjtoken);
}
function fj_get_all_milestone_counts(string $owner, int $ttl, ?string $fjtoken = null): array
{
    $repos = fj_get_all_repos($owner, $ttl, $fjtoken);
    $open = 0;
    $closed = 0;
    ...
    foreach ($repos as $repo) {
        $milestones = fj_get_repo_milestones($owner, $repo['name'], $ttl, $fjtoken);
...
function fj_get_user_repos(string $owner, int $ttl, ?string $fjtoken = null): ?array
{
    global $fjurl;
    $url = "$fjurl/api/v1/users/{$owner}/repos?per_page=100";
    return fj_cached_get($url, $ttl, $fjtoken);
}

// Get all releases of a repo
function fj_get_repo_releases(string $owner, string $repo, int $ttl, ?string $fjtoken = null): ?array
{
    global $fjurl;
    $url = "$fjurl/api/v1/repos/{$owner}/{$repo}/releases?per_page=100";
    return fj_cached_get($url, $ttl, $fjtoken);
}

// Sum downloads of a release
function fj_sum_release_downloads(array $release): int
{
    $sum = 0;
...
// ------------------- Repo Info -------------------
$repoInfo = fj_repo_info($owner, $repo, $ttl, $fjtoken);
...
switch ($metric) {
    ...
    case 'issues-closed':
        ...
        $text2 = fj_repo_issues_count($owner, $repo, 'closed', $ttl, $fjtoken);
        break;

    case 'issues':
        $text1 = $L['issues_total'] ?? 'All Issues';
        $text2 = fj_repo_issues_count($owner, $repo, 'all', $ttl, $fjtoken);
        break;

    case 'issues-allopen':
        $text1 = $L['issues_allopen'] ?? 'Open Issues (All Repos)';
        $text2 = fj_all_repos_issues_count($owner, 'open', $ttl, $fjtoken);
        break;

    case 'issues-all':
        $text1 = $L['issues_all'] ?? 'All Issues (All Repos)';
        $text2 = fj_all_repos_issues_count($owner, 'all', $ttl, $fjtoken);
        break;

    case 'issues-allclosed':
        $text1 = $L['issues_allclosed'] ?? 'Closed Issues (All Repos)';
        $text2 = fj_all_repos_issues_count($owner, 'closed', $ttl, $fjtoken);
        break;

    case 'prs':
        $text1 = $L['prs_total'] ?? 'All PRs';
        $text2 = fj_repo_prs_count($owner, $repo, 'all', $ttl, $fjtoken);
        break;

    case 'prs-open':
        $text1 = $L['prs_open'] ?? 'Open PRs';
        $text2 = fj_repo_prs_count($owner, $repo, 'open', $ttl, $fjtoken);
        break;

    case 'prs-closed':
        $text1 = $L['prs_closed'] ?? 'Closed PRs';
        $text2 = fj_repo_prs_count($owner, $repo, 'closed', $ttl, $fjtoken);
        break;

    case 'prs-all':
        $text1 = $L['prs_all'] ?? 'All PRs (All Repos)';
        $text2 = fj_all_repos_prs_count($owner, 'all', $ttl, $fjtoken);
        break;

    case 'prs-allopen':
        $text1 = $L['prs_allopen'] ?? 'Open PRs (All Repos)';
        $text2 = fj_all_repos_prs_count($owner, 'open', $ttl, $fjtoken);
        break;

    case 'prs-allclosed':
        $text1 = $L['prs_allclosed'] ?? 'Closed PRs (All Repos)';
        $text2 = fj_all_repos_prs_count($owner, 'closed', $ttl, $fjtoken);
        break;
    ...
    case 'language':
        ...
        break;
    case 'release':
        $rel = fj_latest_release($owner, $repo, $ttl, $fjtoken);
        $text1 = $L['release_label'] ?? 'Last release';
        $text2 = $rel['name'] ?? 'Unknown';
        break;
    case 'releases':
        $rels = fj_releases_info($owner, $repo, $ttl, $fjtoken);
    ...
    case 'license':
        $license = fj_repo_license($owner, $repo, $ttl, $fjtoken);
    ...
    case 'username':
        $user = fj_user_info($owner, $ttl, $fjtoken);
        $text1 = $L['name_label'] ?? 'Hello, my name is';
        $text2 = $user['username'] ?? 'Unknown';
        break;

    case 'location':
        $user = fj_user_info($owner, $ttl, $gfjtoken);
    ...
    case 'releases-all':
        ...
        $total = 0;
        foreach ($repos as $repo) {
            $relUrl = "$fjurl/api/v1/repos/{$owner}/{$repo['name']}/releases";
            $rel = fj_cached_get($relUrl, $ttl, $fjtoken);
            if (is_array($rel)) {
                $total += count($rel);
            }
        }
    ...
    case 'lastcommit':
        $info = fj_get_last_commit_info($owner, $ttl, $fjtoken);
    ...
    case 'milestones':
        $milestones = fj_get_repo_milestones($owner, $repo, $ttl, $fjtoken);
    ...
    case 'milestones-all':
        $counts = fj_get_all_milestone_counts($owner, $ttl, $fjtoken);
    ...
    case 'repos':
        $repos = fj_get_user_repos($owner, $ttl, $fjtoken);
        $text1 = $L['repos'] ?? 'Public Repos';
        $text2 = is_array($repos) ? formatNumberShort(count($repos)) : 'N/A';
        break;

    case 'downloads':
        $releases = fj_get_repo_releases($owner, $repo, $ttl, $fjtoken);
        $downloads = 0;
        if (is_array($releases)) {
            foreach ($releases as $rel) {
                $downloads += fj_sum_release_downloads($rel);
            }
        }
    ...
    case 'downloads-latest':
        ...
        if (is_array($releases) && count($releases) > 0) {
            ...
            $downloads = fj_sum_release_downloads($latest);
        }
    ...
    case 'downloads-all':
        ...
        if (is_array($repos)) {
            foreach ($repos as $r) {
                $releases = fj_get_repo_releases($owner, $r['name'], $ttl, $fjtoken);
                if (is_array($releases)) {
                    foreach ($releases as $rel) {
                        $totalDownloads += fj_sum_release_downloads($rel);
                    }
                }
            }
        }
